...
Partial Class deko_2016_intern_bericht_freigabe
    ...
    Protected Sub freigabe_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles freigabe.Click
        ...
        con.Open()
        Try
            command = "update deko_2016_stammdaten set freigabe_bericht_intern = 'Ja' where id = " & stamm_id
            Dim commando As New SqlCommand(command, con)
        ...
        End Try
        Dim Bild_Intern As String = ""
        Dim Bild_Extern As String = ""
        Try
            command = "Select freigabe_bilder_intern from deko_2016_stammdaten where id = " & stamm_id
            Dim commando As New SqlCommand(command, con)
            Bild_Intern = commando.ExecuteScalar

        Catch ex As Exception

        End Try
        Try
            command = "Select freigabe_bilder_extern from deko_2016_stammdaten where id = " & stamm_id
            Dim commando As New SqlCommand(command, con)
            Bild_Extern = commando.ExecuteScalar
        ...
        con.Close()
        con.Dispose()
        If Bild_Extern = "Ja" And Bild_Intern = "Nein" Then
            Response.Redirect("deko_2016_intern_bild_freigabe.aspx?id=" & Request.QueryString.Item("id").ToString)
        Else
    ...
    Protected Sub loeschen_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles loeschen.Click
        ...
        con.Open()
        Try
            command = "update deko_2016_stammdaten set freigabe_bericht_extern = 'Nein' where id = " & stamm_id
            Dim commando As New SqlCommand(command, con)
            commando.ExecuteNonQuery()
        ...
        End Try
        Try
            command = "delete from deko_2016_einsatzbericht_enjoy where stammdaten_id = " & stamm_id
            Dim commando As New SqlCommand(command, con)
            commando.ExecuteNonQuery()
    ...
    Public Sub bilder_laden()
        Dim stamm_id As String = Request.QueryString.Item("id").ToString
        Try
            If IO.File.Exists(Server.MapPath("deko_2016_upload/" & stamm_id & "/bericht.jpg")) Then
    ...
    Public Sub bild_labeltext(ByVal bildname As String, ByVal lab As Label)
        Dim stamm_id As String = Request.QueryString.Item("id").ToString
        lab.Text = lab.Text & "<a href=""http://www.grohe.gate-internet.de/deko_2016_upload/" & stamm_id & "/" & bildname & """ target=""_blank"" style=""color:#000;"">Bericht</a><br />"
    End Sub
End Class
