<%@ Page Title="" Language="VB" MasterPageFile="~/master_intern.master" AutoEventWireup="false" CodeFile="intern_auswertung_mitarbeiter.aspx.vb" Inherits="intern_auswertung_mitarbeiter" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" Runat="Server">
    ...
    <asp:SqlDataSource ID="SqlDataSource1" runat="server" 
        ...
        SelectCommand="SELECT dbo.promotion_login.anmeldename AS Dekorateur, dbo.anzahl_besuche_gesamt.anzahl AS [Anzahl besuchte Outlets], dbo.anzahl_standard.anzahl AS [davon GROHE Standard], dbo.anzahl_spa.anzahl AS [davon GROHE Spa], dbo.[anzahl_zweitanfahrten nach mitarbeiter].menge AS [Anzahl Zweitbesuche], dbo.anzahl_dekorationen_pro_mitarbeiter.menge AS [Anzahl Deko gesamt] FROM dbo.anzahl_standard RIGHT OUTER JOIN dbo.promotion_login LEFT OUTER JOIN dbo.[anzahl_zweitanfahrten nach mitarbeiter] ON dbo.promotion_login.id = dbo.[anzahl_zweitanfahrten nach mitarbeiter].mitarbeiter ON dbo.anzahl_standard.mitarbeiter = dbo.promotion_login.id LEFT OUTER JOIN dbo.anzahl_spa ON dbo.promotion_login.id = dbo.anzahl_spa.mitarbeiter LEFT OUTER JOIN dbo.anzahl_dekorationen_pro_mitarbeiter ON dbo.promotion_login.id = dbo.anzahl_dekorationen_pro_mitarbeiter.mitarbeiter LEFT OUTER JOIN dbo.anzahl_besuche_gesamt ON dbo.promotion_login.id = dbo.anzahl_besuche_gesamt.mitarbeiter WHERE (dbo.promotion_login.projektleiter LIKE @projektleiter)">
        <SelectParameters>
