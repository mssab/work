...
Partial Class deko_2016_extern_eingabe_enjoy
    ...
    Public Sub speichern()
        '*** Command-String ***
        Dim command As String = "INSERT INTO [dbo].[deko_2016_einsatzbericht_enjoy] ([stammdaten_id],[datum],[Banner Eurostyle],[Banner Eurostyle Note],[Banner Livestyle],[Banner Livestyle Note],[Banner Essence],[Banner Essence Note],[Aufkleber Aquatunes],[Aufkleber Aquatunes Note],[Dekosäule Aquatunes],[Dekosäule Aquatunes Note],[Dekowürfel Nachhaltigkeit],[Dekowürfel Nachhaltigkeit Note],[Dekowürfel Made in Germany],[Dekowürfel Made in Germany Note],[Dekowürfel Essence],[Dekowürfel Essence Note],[Dekosäule Logo Standard],[Dekosäule Logo Standard Note],[Aufsteller 5 Jahre Garantie],[Aufsteller 5 Jahre Garantie Note],[Aufkleber QR Code],[Aufkleber QR Code Note],[Aufkleber Logo],[Aufkleber Logo Note],[Badetuch],[Badetuch Note],[anzahl_möglicher_banner],[schaufenster_stromanschluss_vorhanden],[attraktivitaet_der_fenster],[aktivitaet],[aktivitaet_bemerkung],[bemerkung],[telefonische_ankuendigung],[eingang],[keine_deko_generell],[kuenftige_dekos],[versand_gewuenscht],[zweitanfahrt]) values "
        ...
        command = "select MAX(id) from deko_2016_vakanz where stammdaten_id = " & Request.QueryString.Item("id")
        Try
            Dim commando As New SqlCommand(command, con)
        ...
        End Try
        If kontrolle > 0 Then
            command = "update deko_2016_einsatzbericht_enjoy set zweitanfahrt = 'ja' where stammdaten_id = " & Request.QueryString.Item("id")
            Try
                Dim commando As New SqlCommand(command, con)
        ...
        If erfolg = 1 Then
            command = "update deko_2016_stammdaten set freigabe_bericht_extern = 'Ja' where id = " & Request.QueryString.Item("id")
            Try
                Dim commando As New SqlCommand(command, con)
                erfolg = commando.ExecuteNonQuery
    ...
    Protected Sub keine_deko_moeglich_speichern_Click(sender As Object, e As System.EventArgs) Handles keine_deko_moeglich_speichern.Click
        ...
        con.Open()
        Try
            command = "select id from deko_2016_vakanz where stammdaten_id = " & stamm_id
            Dim commando As New SqlCommand(command, con)
    ...
    Public Sub speichern_vakanz(ByVal grund As String, ByVal dat As String, ByVal adr As String)
        '*** Command-String ***
        Dim command As String = "INSERT INTO [dbo].[deko_2016_vakanz] ([stammdaten_id], [datum], [grund], [bis], [neue_adresse], [eingang], [bemerkung], [freigabe_intern]) values "
        ...
        command = "Select MAX(id) from [dbo].[deko_2016_vakanz]"
        Try
            Dim commando As New SqlCommand(command, con)
        ...
        command = "Update [dbo].[deko_2016_stammdaten] set mitarbeiter = 0 where id = " & Request.QueryString.Item("id").ToString
        Try
            Dim commando As New SqlCommand(command, con)
            commando.ExecuteNonQuery()
        ...
        '*** Falls Speichern erfolgreich Vermerk in den Stammdaten ***
        If erfolg = 1 Then
            Response.Redirect("deko_2016_extern_vakanz_bild.aspx?id=" & vakanz)
        Else
            keine_deko_moeglich_bemerkung.Text = command
        End If
    End Sub
