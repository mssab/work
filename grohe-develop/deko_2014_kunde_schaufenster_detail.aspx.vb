...
Partial Class kunde_schaufenster_detail
    ...
    Protected Sub nav_back_Click(sender As Object, e As System.Web.UI.ImageClickEventArgs) Handles nav_back.Click
        ...
        i = i - 1
        Response.Redirect("deko_2014_kunde_schaufenster_detail.aspx?id=" & i)
    End Sub
    ...
    Protected Sub nav_search_Click(sender As Object, e As System.Web.UI.ImageClickEventArgs) Handles nav_search.Click
        Response.Redirect("deko_2014_kunde_schaufenster_liste.aspx")
    End Sub
    ...
    Protected Sub nav_fwd_PreRender(sender As Object, e As System.EventArgs) Handles nav_fwd.PreRender
        ...
        con.Open()
        Try
            command = "select max(id) from deko_2014_stammdaten"
    ...
    Protected Sub Page_PreRender(sender As Object, e As System.EventArgs) Handles Me.PreRender
        ...
        con.Open()
        Try
            command = "select count(id) from deko_2014_stammdaten"
            Dim commando As New SqlCommand(command, con)
            zahl = commando.ExecuteScalar
        ...
        End Try
        stand.Text = "GP-ID &nbsp;&nbsp; " & CInt(Request.QueryString.Item("id")) - 299 & "/" & zahl
        Try
        ...
        End Try
        Try
            command = "select name2 from deko_2014_stammdaten where id =" & stamm_id
            Dim commando As New SqlCommand(command, con)
        ...
        End Try
        Try
            command = "select GP_ID from deko_2014_stammdaten where id =" & stamm_id
            Dim commando As New SqlCommand(command, con)
        ...
        End Try
        Try
            command = "select plz + ' ' + ort as ort from deko_2014_stammdaten where id =" & stamm_id
            Dim commando As New SqlCommand(command, con)
        ...
        End Try
        Try
            command = "select telefon from deko_2014_stammdaten where id =" & stamm_id
            Dim commando As New SqlCommand(command, con)
        ...
        Try
            command = "select glh from deko_2014_stammdaten where id =" & stamm_id
            Dim commando As New SqlCommand(command, con)
        ...
        Try
            command = "select freigabe_bilder_intern from deko_2014_stammdaten where id =" & stamm_id
            Dim commando As New SqlCommand(command, con)
        ...
        End Try
        Try
            command = "select freigabe_bericht_intern from deko_2014_stammdaten where id =" & stamm_id
            Dim commando As New SqlCommand(command, con)
    ...
    Public Sub bilder_laden()
        ...
        If IO.File.Exists(Server.MapPath("deko_2014_upload/" & stamm_id & "/vorher_1.jpg")) Then
            bild_labeltext("vorher_1.jpg", vorher_1)
            zaehler = zaehler + 1
        Else
            vorher_1.Visible = False
        End If
        If IO.File.Exists(Server.MapPath("deko_2014_upload/" & stamm_id & "/vorher_2.jpg")) Then
            bild_labeltext("vorher_2.jpg", vorher_2)
            zaehler = zaehler + 1
        Else
            vorher_2.Visible = False
        End If
        If IO.File.Exists(Server.MapPath("deko_2014_upload/" & stamm_id & "/vorher_3.jpg")) Then
            bild_labeltext("vorher_3.jpg", vorher_3)
            zaehler = zaehler + 1
        Else
            vorher_3.Visible = False
        End If
        If IO.File.Exists(Server.MapPath("deko_2014_upload/" & stamm_id & "/vorher_4.jpg")) Then
            bild_labeltext("vorher_4.jpg", vorher_4)
            zaehler = zaehler + 1
        Else
            vorher_4.Visible = False
        End If
        If IO.File.Exists(Server.MapPath("deko_2014_upload/" & stamm_id & "/nachher_1.jpg")) Then
            bild_labeltext("nachher_1.jpg", nachher_1)
            zaehler = zaehler + 1
        Else
            vorher_1.Visible = False
        End If
        If IO.File.Exists(Server.MapPath("deko_2014_upload/" & stamm_id & "/nachher_2.jpg")) Then
            bild_labeltext("nachher_2.jpg", nachher_2)
            zaehler = zaehler + 1
        Else
            vorher_2.Visible = False
        End If
        If IO.File.Exists(Server.MapPath("deko_2014_upload/" & stamm_id & "/nachher_3.jpg")) Then
            bild_labeltext("nachher_3.jpg", nachher_3)
            zaehler = zaehler + 1
        Else
            vorher_3.Visible = False
        End If
        If IO.File.Exists(Server.MapPath("deko_2014_upload/" & stamm_id & "/nachher_4.jpg")) Then
    ...
    Public Sub bild_labeltext(ByVal bildname As String, ByVal lab As Label)
        Dim stamm_id As String = Request.QueryString.Item("id").ToString
        lab.Text = "<a href=""http://www.grohe.gate-internet.de/deko_2014_upload/" & stamm_id & "/" & bildname & """ target=""_blank"" id=""example1""><img src=""http://www.grohe.gate-internet.de/deko_2014_upload/" & stamm_id & "/" & bildname & """ alt="""" width=""320px"" class=""imgshadow"" /></a>"
        lab.Text = lab.Text & "<br /><br />"
        lab.Text = lab.Text & "<img src=""img/zoomin.gif"" width=""16"" height=""16"" alt="""" />&nbsp;&nbsp;<a href=""http://www.grohe.gate-internet.de/deko_2014_upload/" & stamm_id & "/" & bildname & """ id=""example1"" style=""color:#000;"">vergrößern</a><br /><br />"
    End Sub
    ...
    Public Sub check_bericht(ByVal deko As String, ByVal stamm_id As Integer)
        ...
        If deko = "Standard Deko" Then
            Try
                command = "select aktivitaet from deko_2014_einsatzbericht_enjoy where stammdaten_id = " & stamm_id
                Dim commando As New SqlCommand(command, con)
            ...
            End Try
            Try
                command = "select aktivitaet_bemerkung from deko_2014_einsatzbericht_enjoy where stammdaten_id = " & stamm_id
                Dim commando As New SqlCommand(command, con)
            ...
            End Try
            Try
                command = "select datum from deko_2014_einsatzbericht_enjoy where stammdaten_id = " & stamm_id
                Dim commando As New SqlCommand(command, con)
            ...
            End Try
            Try
                command = "select keine_deko_generell from deko_2014_einsatzbericht_enjoy where stammdaten_id = " & stamm_id
                Dim commando As New SqlCommand(command, con)
            ...
            End Try
            Try
                command = "select versand_gewuenscht from deko_2014_einsatzbericht_enjoy where stammdaten_id = " & stamm_id
                Dim commando As New SqlCommand(command, con)
        ...
        If deko = "SPA Deko" Then
            Try
                command = "select aktivitaet from deko_2014_einsatzbericht_spa where stammdaten_id = " & stamm_id
                Dim commando As New SqlCommand(command, con)
                aktion = commando.ExecuteScalar
            ...
            End Try
            Try
                command = "select aktivitaet_bemerkung from deko_2014_einsatzbericht_spa where stammdaten_id = " & stamm_id
                Dim commando As New SqlCommand(command, con)
                aktion_bemerkung = commando.ExecuteScalar
            ...
            End Try
            Try
                command = "select datum from deko_2014_einsatzbericht_spa where stammdaten_id = " & stamm_id
                Dim commando As New SqlCommand(command, con)
                datum = commando.ExecuteScalar
            ...
            End Try
            Try
                command = "select keine_deko_generell from deko_2014_einsatzbericht_spawhere stammdaten_id = " & stamm_id
                Dim commando As New SqlCommand(command, con)
                generell = commando.ExecuteScalar
            ...
            End Try
            Try
                command = "select versand_gewuenscht from deko_2014_einsatzbericht_spa where stammdaten_id = " & stamm_id
                Dim commando As New SqlCommand(command, con)
                versand = commando.ExecuteScalar
