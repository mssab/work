...
Partial Class extern_eingabe_enjoy
    ...
    Public Sub speichern()
        ...
        values = values & allure.Text.ToString & ", "
        values = values & allure_note.Text.ToString & ", "
        values = values & qr.Text.ToString & ", "
        values = values & qr_note.Text.ToString & ", "
        values = values & power_and_soul.Text.ToString & ", "
        values = values & power_and_soul_note.Text.ToString & ", "
        values = values & woman.Text.ToString & ", "
        values = values & woman_note.Text.ToString & ", "
        values = values & grandera.Text.ToString & ", "
        values = values & grandera_note.Text.ToString & ", "
        ...
        End Try
        If kontrolle > 0 Then
            command = "update einsatzbericht_spa set zweitanfahrt = 'ja' where stammdaten_id = " & Request.QueryString.Item("id")
            Try
                Dim commando As New SqlCommand(command, con)
    ...
    Public Sub speichern_2(ByVal grund As String)
        ...
        If kontrolle > 0 Then
            command1 = "update einsatzbericht_spa set zweitanfahrt = 'ja' where stammdaten_id = " & Request.QueryString.Item("id")
            Try
    ...
    Public Sub speichern_3(ByVal grund As String)
        ...
        Dim kontrolle As Integer = 0
        Dim command1 As String = ""
        command1 = "select MAX(id) from vakanz where stammdaten_id = " & Request.QueryString.Item("id")
        Try
            Dim commando As New SqlCommand(command1, con)
            kontrolle = commando.ExecuteScalar
    ...
    Public Function analyse() As Integer
        ...
        i = check(allure, i)
        i = check_note(allure_note, i)
        i = check(qr, i)
        i = check_note(qr_note, i)
        i = check(power_and_soul, i)
        i = check_note(power_and_soul_note, i)
        i = check(woman, i)
        i = check_note(woman_note, i)
        i = check(grandera, i)
        i = check_note(grandera_note, i)
        i = check(millieu, i)
